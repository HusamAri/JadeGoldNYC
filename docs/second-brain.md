...
- **Kanıtla, varsayma (2026-07):** "İzin kapalı", "veri yok" gibi durum iddialarını
  ...
  **Güçlendirme-3 (2026-08-28) — kural REPO'NUN KENDİ RUNBOOK'una ve "yoksa
  oluştur" denen dış varlıklara da uygulanır; ikisi de bayat olabilir:**
  Vaka-1: `AGENTS.md` "Docker `fuse-overlayfs` ister" diyordu; zorlayınca
  `dockerd` "driver not supported" ile öldü, native `overlayfs` ile sorunsuz
  açıldı — ayrıca hiç bahsedilmeyen `supabase` CLI imajda kurulu DEĞİLDİ
  (ve sürüm önemli: 2.90.0 `config.toml`'daki `[local_smtp]`'yi reddediyor;
  `api.github.com` proxy arkasında 403 verdiği için en güncel sürüm npm'den
  çözülür). Vaka-2: "Ophir Gold USA klasörünü bul ya da OLUŞTUR" istendi;
  arama önce yapıldı ve klasör 6 alt klasörlü, dolu haliyle zaten duruyordu —
  körlemesine oluşturmak ikinci bir boş ağaç açacaktı. Kural: her ön koşulu
  (`which X`, `docker info`) ve her "oluştur" isteğini önce CANLI kontrol/arama
  ile doğrula; runbook'u kanıt değil hipotez say, yanlış çıkanı aynı turda düzelt.
...
- **İndirim mimarisi kurulmadan etiket fiyatı KURULAMAZ; Etsy indirimleri üst
  ...
  17 pozisyonun 8'ini standart tabanda, tamamını Offsite tabanında zarara
  düşürdüğünü gösterdi; yani soru "kaç yüzde" değil "hangi tabana, hangi en
  derin indirime" idi. Üç karar zinciri sırayla çözüldü ve her biri sayıyla
  seçildi: (1) **Offsite Ads** açıktı; aynı kural için etiket I10'da 514'e
  karşı 429, %20 fark. 10k$ altı mağaza çıkabilir, kapatıldı, kural 0,895
  tabanına indi. (2) **Teklifler yığılmaz**: Etsy bir kaleme tek parasal
  indirim uygular (istisna ücretsiz kargo), yani %25 mağaza indirimi açıkken
  %15 sepette-terk kuponu HİÇBİR ŞEY yapmaz; terk/favori teklifi mağaza
  indiriminden DERİN olmak zorunda ve etiket %25'e değil o en derin %30'a
  göre boyutlanır. (3) "Minimum kâr" belirsizdi; en derin indirimde taban
  üstü %10 katkı diye tek sayıya bağlandı. Formül tek satır:
  `ceil_4or9(((landed+0,45)/0,895×1,10)/0,70)`. Yürütme zinciri yine
  aynı: Python ve SQL ızgarayı bağımsız üretip mühür+konum-ağırlıklı
  checksum'da uyuştu (246/246), yazım öncesi canlı mühür okundu (ikinci
  koşu tuzağı), `price-sync ?org=` kuru koşu 246 fark gösterdi ("0 fark"
  tuzağı yok), apply 246 yazdı 0 kaldı, TAZE token'la bağımsız geri okuma
  6×unchanged. Kural: (1) indirim isteği geldiğinde önce "kaç" değil
  "hangi taban + en derin teklif" sorusu çözülür, aksi hâlde yüzde tek başına
  anlamsızdır; (2) dış platformun "üst üste binmez" gibi bir kuralı fiyatın
  GİRDİSİDİR, kampanya notu değil; (3) bir ayar (Offsite) etiketleri %20
  oynatıyorsa fiyat konuşmadan önce o ayar kapatılır ya da bilinçle
  açık bırakılır, karar sayı görülerek verilir. Açık borç: 16 listingin
  10'unda üretici teklifi yok, yalnız gram var; kötümser varsayımla
...
- **"unchanged" ÇIKTISI bir kez daha kimlik hatasıydı; bu kez EKSİK değil
  YANLIŞ EKSEN, ve yanlış taraf PANEL değil ETSY'ydi — düzeltmeden önce
  "hangisi doğru?" sorusunu FOTOĞRAFA sor (2026-09-23):** 10 listing'lik fiyat
  itişinde 9'u yazdı, biri (E08) `unchanged` döndü. Bu repoda `unchanged`
  üçüncü kez sessiz no-op imzası (08-27 yanlış org, 09-17 eşleşmeyen 5 listing).
  Rota yalnız "0 fark" diyordu, NEDEN demiyordu; panelde 3 SKU, Etsy'de 3 SKU,
  ikisi de dolu, yani "anahtar boş mu?" kontrolü TEMİZ geçerdi. Kuru koşuya
  `?detail=1` eklendi (Etsy offering SKU + canlı fiyat + panel karşılığı) ve
  tek çağrı farkı gösterdi: Etsy `16IN/18IN/20IN`, panel `6_5IN/7IN/7_5IN`.
  **İlk düzeltmem YANLIŞ yöndeydi:** Etsy'nin başlığı "Necklace" dediği için
  Etsy'yi doğru sayıp paneli kolyeye çevirdim, fiyatı bastım ve ders bile
  yazdım. Sahip tek cümleyle durdurdu: *"o kolye değil bileklik."* Kanıt
  masamdaydı ve hiç bakmamıştım: listing'in KENDİ fotoğrafları bilezik
  gösteriyordu, `product_type = bracelet` idi, aynı ailenin kardeşi (E10)
  bilezikti. Yani Etsy listing'i 20 Eylül'de yanlış kimlikle (kolye başlığı,
  kolye tag'leri, 16/18/20 inç varyasyon) açılmış, panel doğru kalmıştı.
  Kural: (1) iki kayıt çelişiyorsa "dış sistem doğrudur" varsayımı YOK —
  üçüncü bir kanıta bak (fotoğraf, ürün tipi, kardeş listing), sonra yönü seç;
  (2) bir araç sessiz no-op üretebiliyorsa "0 iş" yanında "neyi neyle
  karşılaştırdım" dökümü de olmalı, tanı aracı olmadan sessiz hata ancak
  şansla bulunur; (3) SKU'nun DOLU olması yetmez, EKSENİ ürün tipiyle tutarlı
  olmalı; (4) yanlış yönde yazılmış bir düzeltme ikinci bir hatadır — panel
  geri alındı, Etsy tarafı `ops/listing-relabel` ile panele eşitlendi (başlık
  + açıklama + tag PATCH, varyasyon etiketi + SKU + fiyat PUT, aynı turda
  geri okuma, panel senkronu). Yan not: 10 listing üretici teklifi olmadan,
