-- 0094 — Gelir semantiği tekleştirme.
-- Denetim bulgusu: Panel/Raporlar/Analizler ciroyu
-- `grand_total_cents || item_total_cents || 0` (JS) semantiğiyle toplarken
-- sales_analytics (0080) yalnız sum(grand_total_cents) topluyordu —
-- grand_total'ı boş (null) gelen kayıtlarda Satışlar özeti panele göre
-- eksik ciro gösteriyordu.
--
-- Bu migration 0080'deki fonksiyonu AYNI imzayla yeniden tanımlar; tek fark
-- üç ciro toplamının coalesce(grand_total_cents, item_total_cents, 0)
-- semantiğine çekilmesi. (getMonthlySalesSeries de kod tarafında aynı
-- semantiğe çevrildi — lib/db/queries/sales.ts.)
...
as $$
  ...
  totals as (
    select
      count(*)                                   as orders,
      -- Panelle aynı gelir semantiği: grand_total yoksa item_total'a düş.
      coalesce(sum(coalesce(grand_total_cents, item_total_cents, 0)), 0)
                                                 as gross_cents,
