...
function json(value: unknown, status = 200) {
  if (new TextEncoder().encode(JSON.stringify(value)).byteLength > MAX_RESPONSE_BYTES) {
    return NextResponse.json(
      { error: "Page is too large. Retry with a smaller page_size." },
      { status: 413, headers: { "Cache-Control": "private, no-store", Vary: "Cookie" } },
    );
  }
...
export async function GET(request: Request) {
  ...
  if (!isManager(membership.role)) return json({ error: MANAGER_ONLY_ERROR }, 403);

  const sp = new URL(request.url).searchParams;
  const entity = sp.get("entity") ?? "product_variants";
  if (entity !== "product_variants" && entity !== "products") {
    return json({ error: "entity must be product_variants or products." }, 400);
  }
  const sinceInput = sp.get("since");
  const untilInput = sp.get("until");
  if ((sinceInput === null) !== (untilInput === null)) {
    return json({ error: "Provide both since and until, or neither for summary." }, 400);
  }
  ...
  if (sinceInput !== null && (!since || !requestedUntil || Date.parse(since) >= Date.parse(requestedUntil))) {
    return json({ error: "Use valid ISO timestamps with timezones and since earlier than until." }, 400);
  }
  const cursor = sp.get("cursor");
  if (cursor !== null && (!since || !UUID.test(cursor))) {
    return json({ error: "cursor requires a date window and must be an audit UUID." }, 400);
  }
  ...
  if (!/^\d+$/.test(sizeInput) || !Number.isInteger(pageSize) || pageSize < 1 || pageSize > MAX_PAGE_SIZE) {
    return json({ error: `page_size must be between 1 and ${MAX_PAGE_SIZE}.` }, 400);
  }
  ...
  if (since && Date.parse(since) >= Date.parse(until)) {
    return json({ error: "since must precede the current audit cutoff." }, 400);
  }
  ...
  try {
    ...
    const { data: org, error: orgError } = await client.from("organizations")
      ...
      .maybeSingle();
    if (orgError) return json({ error: "Active organization could not be verified." }, 500);
    if (!org || org.slug !== OPHIR_SLUG) {
      return json({ error: "Active organization must be Ophir Gold USA." }, 409);
    }
    ...
    if (!since) {
      ...
      while (scanned < MAX_WINDOW_ROWS && changes.length < SUMMARY_EVENTS) {
        ...
        if (error) return json({ error: "Pricing audit summary could not be read." }, 500);
    ...
    if (countError || total === null) return json({ error: "Audit window count could not be read." }, 500);
    if (total > MAX_WINDOW_ROWS) {
      return json({ error: "Audit window exceeds the safety bound. Narrow since/until.", total_update_records: total, max_update_records: MAX_WINDOW_ROWS }, 413);
    }
    const rows: AuditRow[] = [];
    let after = cursor;
    let complete = false;
    while (rows.length < pageSize) {
      ...
      const { data, error } = await query;
      if (error) return json({ error: "Pricing audit page could not be read." }, 500);
    ...
    if (!complete && after) {
      ...
      if (error) return json({ error: "Audit page completion could not be verified." }, 500);
    ...
    return json({ error: "Pricing audit export could not be completed." }, 500);
  }
}
