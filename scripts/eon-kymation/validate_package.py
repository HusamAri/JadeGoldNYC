#!/usr/bin/env python3
"""Validate the complete EON Kymation listing package and render QA sheets."""
...
PANEL_REFRESH_PATH = ROOT / "docs/eon/kymation-1009/panel-refresh.sql"
WEIGHTS_PATH = ROOT / "docs/eon/eon-weight-tables.json"
...
EXPECTED_WIDTHS = list(range(4, 13))
EXPECTED_SIZES = [str(value / 2).removesuffix(".0") for value in range(8, 33)]
FORBIDDEN_CHARS = {"\u2013": "en dash", "\u2014": "em dash", "\u00e2": "circumflex a"}
SKU_PATTERN = re.compile(r"(?:GLD|WHG|RSG)-R-(?:10|14|18)09-(?:[4-9]|1[0-2])MM-(?:[4-9]|1[0-6])(?:\.5)?")
URL_PATTERN = re.compile(r"https://amuletta\.artifactstudio\.info/eon/kymation-1009/[a-z0-9-]+/[a-z0-9-]+\.jpg")
...
def main() -> None:
    ...
    checks.check(
        "shop_section",
        all(listing["shop_section"] == "Frieze & Textured Bands" for listing in listings),
        {listing["family_sku"]: listing["shop_section"] for listing in listings},
    )
    checks.check(
        "labor_usd",
        all(listing["pricing"]["labor_usd"] == 55.0 for listing in listings),
        {listing["family_sku"]: listing["pricing"]["labor_usd"] for listing in listings},
    )
    ...
    checks.check("variants_per_listing", all(count == 225 for count in variant_counts.values()), variant_counts)
    checks.check("variant_total", len(all_variant_skus) == 2025, len(all_variant_skus))
    checks.check("unique_variant_skus", len(set(all_variant_skus)) == 2025, len(set(all_variant_skus)))
    checks.check(
        "variant_dimensions",
    ...
    weights = json.loads(WEIGHTS_PATH.read_text())["weights_grams"]
    weight_mismatches: list[dict[str, object]] = []
    for listing in listings:
        karat = listing["karat"]
        for variant in listing["variants"]:
            width = str(variant["width_mm"])
            size = float(variant["ring_size_us"])
            row = weights[karat][width]["1.5"]
            if size.is_integer():
                expected = float(row[int(size) - 4])
            else:
                lower = int(size)
                expected = round((float(row[lower - 4]) + float(row[lower - 3])) / 2, 3)
            actual = float(variant["weight_grams"])
            if actual != expected:
                weight_mismatches.append(
                    {"sku": variant["sku"], "expected": expected, "actual": actual}
                )
    checks.check(
        "weight_table_parity",
        not weight_mismatches,
        {"mismatch_count": len(weight_mismatches), "examples": weight_mismatches[:10]},
    )

    price_ranges: dict[str, dict[str, float]] = {}
    pricing_ok = True
    for listing in listings:
        prices = [variant["price_preview_cents_active_basis"] for variant in listing["variants"]]
        pricing_ok = pricing_ok and all(price > 0 and price % 500 == 0 for price in prices)
        ordered = sorted(
            listing["variants"],
            key=lambda variant: (
                variant["width_mm"],
                float(variant["ring_size_us"]),
            ),
        )
        for width in EXPECTED_WIDTHS:
            width_prices = [
                ...
                if variant["width_mm"] == width
            ]
            pricing_ok = pricing_ok and width_prices == sorted(width_prices)
        price_ranges[listing["family_sku"]] = {
            "minimum_usd": min(prices) / 100,
            "maximum_usd": max(prices) / 100,
        }
    checks.check("price_profile", pricing_ok, price_ranges)
    ...
    manual_review = {
        ...
        "gates": {
            "product_geometry": "Broad fine satin center, twin short-tool-mark milgrain lines, planar mirror bevels and polished comfort-fit interior match the source ring.",
            "profile": "Side-profile photographs expose the narrow 1.5 mm annular wall without making the band chunky.",
            "karat_tone": "10K, 14K and 18K use distinct, credible metal tones and distinct scenes within each metal family.",
            "natural_shadows": "Product photographs show believable contact shadows, restrained color and physically plausible glass refraction.",
            "greek_roots": "Kymation references remain abstract through repeating line rhythm and geometric glass, without literal costume props.",
            "cards": "Information cards are legible, SKU-specific and preserve all production facts.",
        },
    }

    report = {
        "package": "EON Kymation 1009",
        ...
        "pricing_note": {
            ...
            "profile": "55 USD labor, fire allowance, packaging, shipping, width multiplier, sale gross-up and 5 USD rounding.",
        },
        "artifacts": {
