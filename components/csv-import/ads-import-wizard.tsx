...
export function AdsImportWizard() {
  ...
  /** Dışa aktarımın kapsadığı tarih aralığı — etikette saklanır. */
  ...
  function commit() {
    start(async () => {
      const r = await commitAdsImport(mapped, periodNote);
      if (r.error) {
        toast.error(r.error);
        return;
      }
      toast.success(
        `${r.imported} satır içe aktarıldı — ${r.matchedById} kimlikle, ` +
          `${r.matchedByTitle} başlıkla eşleşti, ${r.unmatched} eşleşmedi.`,
      );
      router.refresh();
    });
  }
        ...
        <CardContent className="space-y-4">
          ...
          {headers.length > 0 && (
            <div className="grid gap-3 sm:grid-cols-3">
              {colSelect("listingId", "Listing kimliği — önerilen")}
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <p className="text-muted-foreground text-sm">
                ...
                önce listing kimliği, kimlik yoksa başlık üzerinden bağlanır.
              </p>
              <div className="max-w-sm space-y-1.5">
                <Label htmlFor="ads-period">Dönem notu — ops.</Label>
                <Input
                  id="ads-period"
                  value={periodNote}
                  onChange={(e) => setPeriodNote(e.target.value)}
                  placeholder="2026-08-01…2026-08-31"
                />
                <p className="text-muted-foreground text-xs">
                  Dışa aktarımın kapsadığı aralık. Etikete eklenir
                  (&quot;son 30 · …&quot;) — hangi pencereden geldiği kayıtta
                  kalır, mevcut raporlar etiketi görmeye devam eder.
                </p>
              </div>
            </div>
